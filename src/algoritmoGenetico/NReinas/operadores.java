...
import algoritmoGenetico.binario.*;
...
public class operadores 
{
    ...
    public static individuo seleccionAleatoria(ArrayList<individuo> pob)
   {
    ...
    {
        //int ran = ThreadLocalRandom.current().nextInt(0, n)-1; //n

        //ind.getCromosoma()[ran] = ThreadLocalRandom.current().nextInt(0, n); 
        Random r = new Random();
        int pos = r.nextInt(n);
        if(pos < n)
        {
            ind.getCromosoma()[pos] = r.nextInt(n);
            ind.calculaFitness();
        }
    ...
    public static int[] mascaraAleatoria(int n)
    {
        ...
        {
            aux[i] = ThreadLocalRandom.current().nextInt(0, 2); //n+1
        }
